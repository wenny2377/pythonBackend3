...
"""
    ...
    def _generate_answer(self, query, intent_type, user_id, room,
        ...
        intent_hint = {
            "fuzzy_need":    "User is expressing a need/feeling. Suggest where to go and what to use based on their personal habits. Include a recommendation reason.",
            "object_search": "User is looking for an object. Tell them where it was last seen.",
            "habit_query":   "User is asking about someone's habits. Summarize based on memory.",
            "direct":        "Answer the question directly and concisely.",
        }.get(intent_type, "")

        lang = "Traditional Chinese (繁體中文)" if is_chinese else "English"

        prompt = f"""You are a warm, personalized home service robot for {user_id}.
{intent_hint}

Reply in {lang}. 2-3 sentences max. Be warm and specific.
If recommending something, mention WHY (based on their habits).
Do NOT repeat the user's words back. No preamble.
