...
def plot_confusion_matrix(cm, labels, out_path, overall_acc, macro_f1):
    ...
    fig, ax = plt.subplots(
        figsize=(max(7, len(active) * 1.1), max(6, len(active) * 1.0)))
...
def save_summary(rows, macro_f1, overall_acc, n_total, out_path):
    ...
    for r in active:
        lines.append(
            f"  {r['label']:14s}  P={r['precision']:.3f}  "
...
def main():
    ...
    if not docs:
        print("No eval_logs found. Ensure /predict is writing eval_logs.")
        return
