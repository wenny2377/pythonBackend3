...
從 manifold_points 讀取特徵向量，執行 UMAP + HDBSCAN，
產出三張並排散布圖和 Silhouette Score。
...
前提：
    Experiment3 mode 跑完（120 episodes），
    ManifoldEngine 已至少 refit 一次（每 50 筆觸發）。

輸出：
    exp3_umap.png       → 論文 Figure（三欄並排）
    exp3_summary.txt    → 論文文字摘要（含 Silhouette Score）
"""
...
def run_umap_hdbscan(X):
    n = len(X)
    print(f"  Running StandardScaler...")
    ...
    if valid.sum() > 1 and n_clusters > 1:
        ...
        print(f"  ⚠️  Silhouette not computable (clusters={n_clusters})")
...
def plot(embedding, actions, slots, prevs, labels,
    ...
    ax1 = axes[0]
    observed_behaviors = [b for b in BEHAVIOR_COLORS if any(a == b for a in actions)]
    ...
    ax2 = axes[1]
    observed_slots = [s for s in SLOT_COLORS if any(sl == s for sl in slots)]
    ...
    # Annotate cluster centers with dominant behavior
    for cid in unique_labels:
        if cid == -1: continue
        mask    = labels == cid
        pts     = embedding[mask]
    ...
    ax3.set_xlabel("UMAP dim 1"); ax3.set_ylabel("UMAP dim 2")
    ...
    plt.close()
    print(f"  ✅ UMAP plot saved: {out_path}")
...
def save_summary(actions, slots, labels, n_clusters, s_score, n_total, out_path):
    ...
    lines = [
        ...
        f"Silhouette Score S:  {s_score:.4f}" if s_score else "Silhouette Score S:  N/A",
        ...
        *[f"  {k}: {v}" for k, v in Counter(slots).most_common()],
        ...
        f"The UMAP manifold projection of {n_total} behavioral observations",
        f"(1,158-dim feature vectors) produces {n_clusters} distinct clusters",
        f"identified by HDBSCAN, achieving a mean Silhouette Score of",
        f"S = {s_score:.4f}" if s_score else "S = N/A (insufficient clusters)",
    ]
    if s_score:
        lines += [
            f"({'≥' if passed else '<'} 0.50 acceptance threshold).",
            f"Each cluster corresponds to a coherent behavioral habit",
            f"combining action semantics, temporal context, and spatial",
            f"location, confirming that the training-free 1,158-dim",
            f"feature vector captures the latent structure of user",
            f"behavioral habits without any gradient-based training (RQ4).",
        ]

    with open(out_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  ✅ Summary saved: {out_path}")
    print(f"\n  Silhouette Score S = {s_score:.4f}  →  {status}" if s_score
...
def main():
    ...
    if X is None or len(X) < 10:
        print("❌ Not enough data in manifold_points.")
        print("   Minimum 10 required. Run Experiment3 first.")
        sys.exit(1)
    ...
    plot(
        embedding, actions, slots, prevs, labels,
        n_clusters, s_score, len(X),
        out_path=os.path.join(args.out, "exp3_umap.png"),
    )
    save_summary(
        actions, slots, labels, n_clusters, s_score, len(X),
