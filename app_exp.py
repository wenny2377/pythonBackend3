...
def predict():
    try:
        ...
        activity     = data.get('activity', '')       # Unity ground truth
...
def exp_checkpoint():
    """
    Unity ExperimentRunner 每 5 次呼叫一次（實驗三A）或每天結束時（實驗三B）。
    記錄當下的 weight 值與 FAISS similarity，供實驗三折線圖使用。
    """
    try:
        ...
        if experiment == "exp3a":
            # observation_logs 欄位：user, action, weight（已確認）
            ...
            try:
                results = vector_memory.search_habit(
                    f"{user} {action}", user_id=user, top_k=1
                )
