...
class ObjectClassifier:
    ...
    def _find_closest_furniture(self, x, z, room, current_anchor=None):
        ...
        if current_anchor and current_anchor in self._furniture_cache:
            ...
            f_pos = f.get("pos")
            if f_pos and math.sqrt((x - f_pos[0])**2 + (z - f_pos[1])**2) <= INERTIA_THRESHOLD:
    ...
    def _make_dynamic_op(self, doc):
        ...
        if old:
            old_anchor = old.get("last_seen_on")
            old_rel = old.get("spatial_rel")
        else:
            old_anchor, old_rel = None, None
        ...
        if old_anchor and anchor != old_anchor:
            print(f"[Classifier] Anchor Change: {label} ({old_anchor} -> {anchor}) | dist_to_user: {u_dist:.2f}m")
